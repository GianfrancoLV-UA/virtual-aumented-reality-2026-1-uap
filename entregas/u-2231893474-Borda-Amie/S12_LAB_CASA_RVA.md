...
| Campo | Detalle |
|-------|---------|
| **Estudiante** | Amie Paola Borda Garcia |
| **Código** | 2231893474 |
...
═══════════════════════════════════════════════════════
Fecha/hora de medición:05/07/2026, 22:16
FPS (promedio durante 30 seg de juego): 303FPS
FPS (mínimo durante 30 seg): 270FPS
Draw Calls: 5
Tris renderizados: 162
Verts renderizados: 432
Used Memory: 10.6 MB
═══════════════════════════════════════════════════════
```
...
═══════════════════════════════════════════════════════
Tiempo total del frame: 77.33 ms
CPU time: 77.33 ms
GPU time: _______________ ms

Top 5 procesos por tiempo:
1. EditorLoop (Overhead del editor) : 74.01 ms
2. PlayerLoop (Lógica del juego)    : 0.57 ms
3. Camera.Render (Renderizado)      : 0.45 ms
4. ScriptRunBehaviourUpdate         : 0.12 ms
5. FlushCounters                    : 0.08 ms

¿CPU bound o GPU bound?: CPU Bound por el alto tiempo en EditorLoop
...
═══════════════════════════════════════════════════════
Total de GameObjects activos: 7
Objetos que NO se mueven (candidatos Static Batching): 1
Objetos que SÍ se mueven: 0
Materiales únicos en la escena: 1
...
La técnica con más potencial es Static Batching. Como la escena de AR es pequeña y los objetos como el cubo o el mobiliario virtual son estáticos, marcarlos como Static permitiría reducir los Draw Calls a un solo proceso, optimizando el uso de la CPU en móviles.
...
4. Verificar con Stats + Profiler:
   ...
   ═══════════════════════════════════════════════
   Objetos marcados como Static: 2
   FPS antes: 303 | FPS después: 305
   Draw Calls antes: 6 | después: 5
   Cambio en Tris: 0
   ═══════════════════════════════════════════════
   ```
...
11. Medir:
    ...
    ═══════════════════════════════════════════════
    FPS con cámara CERCA del objeto:294 FPS
    FPS con cámara LEJOS del objeto: 315 FPS
    Tris CERCA:174 | LEJOS: 138
    ═══════════════════════════════════════════════
    ```
...
Es más útil cuando el usuario camina hacia el objeto virtual.

En Realidad Aumentada, la distancia entre la cámara y el objeto es dinámica y cambia constantemente según el movimiento físico del usuario. Cuando el usuario se aleja, el objeto ocupa una porción mínima de la pantalla y no es necesario procesar todos sus polígonos. El LOD Group permite que el hardware móvil celular ahorre recursos de procesamiento y batería al renderizar versiones simplificadas automáticamente, manteniendo la fluidez (FPS) sin que el usuario note la pérdida de detalle a la distancia.
---
...
4. Anota el nombre: `_AppLogic.cs`
...
1. Búsqueda de objetos con GameObject.Find en cada frame para detectar el marcador.
2. Llamadas repetitivas a GetComponent para actualizar componentes de UI.
3. Creación de nuevos strings cada frame para mostrar mensajes al usuario.
```
...
2. Medir con Profiler:
   ...
   ══════════════════════════════════════════════════
   Script original   — tiempo: 0.85 ms | GC Alloc: 128 bytes
   Script optimizado — tiempo: 0.00_ ms | GC Alloc: 0 bytes
   Reducción de tiempo: 0.85 ms (100%)
   Reducción de GC Alloc: 128 bytes (100%)
   ══════════════════════════════════════════════════
   ```
...
══════════════════════════════════════════════════
Con NivelCarga = 0:    FPS promedio = 120
Con NivelCarga = 50:   FPS promedio = 55
Con NivelCarga = 100:  FPS promedio = 30
...
Mensajes en Console:
- [23:57:42] ⚠️ FPS Bajos: Reduciendo calidad...
- [23:58:02] FPS Estables: Restaurando calidad...
══════════════════════════════════════════════════
```
...
El principal riesgo de evaluar cada frame es el "jittering" o parpadeo visual. Si el rendimiento oscila levemente alrededor del umbral (por ejemplo, entre 59 y 61 FPS), el sistema activaría y desactivaría las optimizaciones constantemente en cada frame, lo cual es visualmente molesto para el usuario y consume recursos extra de CPU.
...
Integraría el AdaptiveQualityManager creando un objeto vacío "GlobalManager" que persista en la escena de escaneo AR. Configuraría el script para que monitoree el rendimiento especialmente cuando se detecta el marcador (Target) y aparecen los modelos 3D complejos.

En Ecodex, los primeros objetos en reducir calidad serían:
1.- Sistemas de Partículas: Los efectos visuales de "reciclaje exitoso" o chispas.
2.- Sombras de la Lata/Objeto AR: Reduciría la distancia de sombra o la desactivaría, ya que en AR móvil las sombras dinámicas son muy costosas.
3.- Post-processing: Si usamos Bloom o Color Grading, serían los primeros en desactivarse.
4.- Textos flotantes secundarios: Reducir la frecuencia de actualización de la UI no crítica.
